...
#' @param geography Required -- at what geography should the data be aggregated? Must be `county`.
#' @param file_path The path from which to read the dataset if `api = FALSE`.
#' @param api Included for consistency; must be FALSE.
#'
#' @returns A dataframe comprising month x year x county counts of hazard events.
#' @export
#'
#' @examples
#' \dontrun{
...
get_sheldus_county = function(
	geography = "county",
	file_path = file.path(
		get_box_data_path(), "hazards", "sheldus",
		"SHELDUS_22.0_01012916_01012023_AllCounties_CountyAggregate_YearMonth_2022USD",
		"UID14148f_AGG_A.csv"),
	api = FALSE) {

	## is a valid geography option provided?
	if (!geography == "county") {
		stop("Invalid geography. Valid options are 'county'.")}

  ## no API exists; this parameter must be FALSE.
	if (api) {
		stop("There is no API available for these data. Please set `api = FALSE`.")}
  ...
  benchmark_geographies = dplyr::bind_rows(
	...
	df1 = file_path %>%
		...
		dplyr::rename_with(.cols = dplyr::everything(), .fn = ~ stringr::str_replace_all(.x, c("dmg" = "damage", "adj" = "adjusted"))) %>%
		dplyr::select(
			unique_id,
			GEOID,
			state_name,
...
"The unit of observation is: ", geography, " x year x month. ",
